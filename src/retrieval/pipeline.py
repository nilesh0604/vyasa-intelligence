    ...
    def _perform_hybrid_search(
        ...
        if use_bm25 and use_dense:
            ...
            fused_results = self.rank_fusion.reciprocal_rank_fusion(
                ...
                top_k=top_k,
                weights=[
                    strategy.get("bm25_weight", 0.5),
                    strategy.get("dense_weight", 0.5),
                ],
            )
            ...
            for field, allowed_values in filters.items():
                if field in metadata:
                    ...
                    if isinstance(value, list):
                        ...
                        if not any(
                            v.lower() in [av.lower() for av in allowed_values]
                            for v in value
                        ):
                            include = False
                            break
    ...
    def get_pipeline_statistics(self) -> Dict:
        ...
        stats = {
            ...
            "models": {
                "embedding_model": self.embedding_model,
                "reranker_model": (
                    self.reranker_model if self.enable_reranking else None
                ),
                "device": self.device,
            },
        }
